...
private fun WifiSecurityProfile.pmfFinding(): SecurityFinding? = when (managementFrameProtection) {
    ManagementFrameProtection.REQUIRED -> SecurityFinding(
        Severity.INFO,
        "Protección de tramas activa",
        "La red exige protección de tramas de gestión (PMF), lo que dificulta ataques de desconexión.",
    )
    ManagementFrameProtection.DISABLED -> SecurityFinding(
        Severity.LOW,
...
class OpenNetworkAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment = SecurityAssessment(
        rating = SecurityRating.INSECURE,
        headline = "Red abierta",
        plainExplanation = "Cualquiera puede conectarse y el tráfico viaja sin cifrar. Evita introducir " +
...
class LegacyNetworkAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment {
        ...
        return SecurityAssessment(
            ...
            plainExplanation = "Este mecanismo está roto desde hace años y puede vulnerarse con herramientas " +
                "comunes. No debería usarse.",
            technicalSummary = if (wep) {
...
class PersonalNetworkAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment {
        val (rating, headline, plain) = when (profile.family) {
            SecurityFamily.WPA3_PERSONAL -> Triple(
                SecurityRating.HIGH,
                "WPA3-Personal",
                "Cifrado moderno y robusto para redes domésticas. Buena elección.",
            )
            SecurityFamily.WPA2_WPA3_PERSONAL -> Triple(
                SecurityRating.HIGH,
...
class EnterpriseNetworkAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment {
        ...
        return SecurityAssessment(
            ...
            plainExplanation = "Autenticación individual mediante servidor (802.1X). Habitual en empresas y " +
...
class OweAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment = SecurityAssessment(
        ...
        plainExplanation = "Red sin contraseña pero con el tráfico cifrado de forma individual. Mejor que una " +
...
class PasspointAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment = SecurityAssessment(
        ...
        plainExplanation = "Conexión automática y cifrada a redes de operadores mediante credenciales " +
...
class DppAssessmentStrategy : SecurityAssessmentStrategy {
    ...
    override suspend fun assess(profile: WifiSecurityProfile): SecurityAssessment = SecurityAssessment(
        ...
        plainExplanation = "Aprovisionamiento moderno mediante claves públicas (por ejemplo, códigos QR), sin " +
