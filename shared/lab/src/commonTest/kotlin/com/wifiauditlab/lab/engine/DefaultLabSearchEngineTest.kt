...
class DefaultLabSearchEngineTest {
    ...
    @Test
    fun finds_the_hidden_secret_and_reveals_it() = runTest {
    ...
    @Test
    fun stops_with_attempt_limit_when_secret_is_out_of_reach() = runTest {
    ...
    @Test
    fun secret_found_exactly_at_the_attempt_limit_is_still_found() = runTest {
    ...
    @Test
    fun secret_one_past_the_limit_is_reported_as_limit_reached() = runTest {
    ...
    @Test
    fun stops_with_duration_limit() = runTest {
        val challenge = LabChallenge.withKnownSecret(Alphabet.DIGITS, secret = "99")
    ...
    @Test
    fun cancel_while_running_emits_cancelled_almost_immediately() = runTest {
        val challenge = LabChallenge.withKnownSecret(Alphabet.DIGITS, secret = "99")
        val engine = DefaultLabSearchEngine()
        ...
        val events = engine.run(
            ...
            CancelAfterPolls(0), // cancelled at the very first batch boundary
        ).toList()
    ...
    @Test
    fun cancel_across_bucket_transition() = runTest {
        val challenge = LabChallenge.withHiddenSecret(
            Alphabet.of("ab"),
            LengthPolicy(1, 3),
            seed = 7,
        )
    ...
    @Test
    fun completes_without_finding_when_secret_is_outside_the_plan() = runTest {
        ...
        assertEquals(CombinationCount.of(2), completed.metrics.attempts) // "a", "b"
    }

    @Test
    fun emits_preparing_and_started_before_terminal_event() = runTest {
        val challenge = LabChallenge.withKnownSecret(Alphabet.DIGITS, secret = "07")
        val engine = DefaultLabSearchEngine()
